...
public class TranslucentUtils {
    ...
    //Android15以上默认强制开启状态栏导航栏沉浸，所以本方法不推荐使用，故而转为private
    private static void setTranslucentTOP(Window window){
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.VANILLA_ICE_CREAM){
    ...
    //Android15以上默认强制开启状态栏导航栏沉浸，所以本方法不推荐使用，故而转为private
    private static void setTranslucentBottom(Window window){
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.VANILLA_ICE_CREAM){
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
